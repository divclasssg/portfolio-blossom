          ...
          <table className={styles.table}>
            <thead>
              <tr>
                ...
                <th scope="col" className={styles['col-diagnosis']}>진단명</th>
                <th scope="col" className={styles['col-hospital']}>진료기관</th>
                <th scope="col" className={styles['col-type']}>진료 유형</th>
                <th scope="col" className={styles['col-action']} aria-label="상세 보기" />
              </tr>
            </thead>
            <tbody>
              {sorted.map((visit, i) => {
                ...
                return (
                  ...
                  >
                    <td className={styles['col-date']}>{formatDate(visit.date)}</td>
                    <td className={styles['col-diagnosis']}>{visit.diagnosis}</td>
                    <td className={styles['col-hospital']}>{visit.hospital}</td>
                    <td className={styles['col-type']}>
                      <span className={styles['type-badge']}>{d.visitType}</span>
                    </td>
                    <td className={styles['col-action']} aria-hidden="true">
                      <span className={`${styles.chevron} ${selectedIdx === i ? styles['chevron--open'] : ''}`}>
                        ›
                      </span>
                    </td>
                  </tr>
            ...
            {detail.prescriptions.length > 0 && (
              <ul className={styles['prescription-list']}>
                {detail.prescriptions.map((p, i) => (
                  <li key={i} className={styles['prescription-item']}>{p}</li>
                ))}
              </ul>
            )}
          </div>
