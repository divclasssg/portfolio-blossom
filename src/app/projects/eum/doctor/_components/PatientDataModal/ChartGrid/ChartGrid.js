...
function buildNrsData(symptoms, domain, activeCategory) {
  const map = {};
  symptoms.data.forEach((item) => { map[item.date] = item; });
...
function buildSleepData(sleep, domain) {
  const map = {};
  sleep.data_7day.forEach((item) => { map[item.date] = item; });
  return domain.map((date) => ({
    date,
...
function buildHrData(heartRate, domain) {
  const map = {};
  heartRate.data.forEach((item) => { map[item.date] = item; });
  return domain.map((date) => ({
    date,
...
function buildBpData(bp, domain) {
  const map = {};
  bp.data_7day.forEach((item) => { map[item.date] = item; });
  return domain.map((date) => {
