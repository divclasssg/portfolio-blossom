    ...
    <>
      ...
      <main className={styles['page']}>
        <section className={styles['content']} aria-labelledby="health-info-title">
          ...
          {/* 기본 건강 정보 섹션 */}
          <div className={[styles['section-header'], styles['section-header-first']].join(' ')}>
            <span className={styles['section-title']}>기본 건강 정보</span>
            <span className={styles['required-badge']} aria-label="필수 항목">필수</span>
          </div>
          ...
          <div className={styles['field-group']}>
            ...
            <div className={styles['input-unit-row']}>
              <input
                ...
                type="number"
                inputMode="decimal"
                className={[styles['input'], heightError ? styles['input-error'] : ''].filter(Boolean).join(' ')}
                value={height}
            ...
            {heightError && (
              <span id="height-error" className={styles['error']} role="alert">{heightError}</span>
            )}
          </div>
          ...
          <div className={styles['field-group']}>
            ...
            <div className={styles['input-unit-row']}>
              <input
                ...
                type="number"
                inputMode="decimal"
                className={[styles['input'], weightError ? styles['input-error'] : ''].filter(Boolean).join(' ')}
                value={weight}
            ...
            {weightError && (
              <span id="weight-error" className={styles['error']} role="alert">{weightError}</span>
            )}
          </div>
          ...
          <div className={styles['section-header']}>
            ...
            <span className={styles['optional-badge']} aria-label="선택 항목">선택</span>
          </div>

          {/* 기저질환 */}
          <div className={styles['field-group']}>
            <span className={styles['label']} id="conditions-label">기저질환</span>
            <div className={styles['input-row']} role="group" aria-labelledby="conditions-label">
              <input
                type="text"
                className={styles['input']}
                value={conditionInput}
                onChange={(e) => { setConditionInput(e.target.value); setConditionDupError(''); }}
            ...
            {conditionDupError && (
              <span className={styles['error']} role="alert">{conditionDupError}</span>
            )}
            {conditions.length > 0 && (
          ...
          {/* 알레르기 */}
          <div className={styles['field-group']}>
            <span className={styles['label']} id="allergies-label">알레르기</span>
            <div
            ...
            >
              <input
                ...
                className={styles['input']}
                value={allergenInput}
                onChange={(e) => { setAllergenInput(e.target.value); setAllergyDupError(''); }}
            ...
            {allergyDupError && (
              <span className={styles['error']} role="alert">{allergyDupError}</span>
            )}
            {allergies.length > 0 && (
                ...
                {allergies.map((item, i) => (
                  <li key={i} className={styles['chip']}>
                    <span>
                      {item.allergen}
                      {item.reaction && (
                        <span className={styles['chip-reaction']}> ({item.reaction})</span>
                      )}
                    </span>
                    <button
                      type="button"
