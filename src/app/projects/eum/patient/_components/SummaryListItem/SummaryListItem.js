...
export default function SummaryListItem({ sessionId, visitDate, hospitalName, doctorName, diagnosisName }) {
  return (
    <Link href={`/projects/eum/patient/summary/${sessionId}`} className={styles['item']}>
      <p className={styles['meta']}>{visitDate} · {hospitalName} · {doctorName}</p>
