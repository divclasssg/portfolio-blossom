...
function extractJsonTag(content, tagName) {
    ...
    for (let i = jsonStart; i < content.length; i++) {
        ...
        if (depth === 0) {
            try {
                ...
                console.warn(
                    `[chat/route] ${tagName} 태그 JSON 파싱 실패:`,
                    content.slice(jsonStart, i + 1)
                );
                return null;
            }
        }
    ...
    console.warn(
        `[chat/route] ${tagName} 태그 닫힘 없음:`,
        content.slice(startIdx, startIdx + 100)
    );
    return null;
}
            ...
            try {
                ...
                for await (const chunk of completion) {
                    ...
                    while (pending.length > 0) {
                        ...
                        if (
                            potentialTag.startsWith('[META:') ||
                            potentialTag.startsWith('[DONE:')
                        ) {
                            // 태그 확인 — 전송하지 않고 소비
