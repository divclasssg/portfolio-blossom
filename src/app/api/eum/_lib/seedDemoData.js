 ...
 * @param {string} patientId - pat_demo_xxxxxxxxxxxx
 * @returns {{ latestSessionId: string }}
 */
export async function seedDemoScenario(supabase, patientId) {
  // suffix: pat_demo_ 이후 12자
  const suffix = patientId.replace('pat_demo_', '');

  // ── 1. sessions 삽입 ─────────────────────────────────────────
  const sessionRows = sessionsJson.sessions.map((s) => {
    const newId = remapSessionId(s.session_id, suffix);
    const row = {
      id: newId,
      ...
      referral_from: s.referral_from
        ? {
            ...s.referral_from,
            session_id: remapSessionId(s.referral_from.session_id, suffix),
          }
        : null,
      status: s.status,
  ...
  sessionsJson.sessions.forEach((s) => {
    const newSessionId = remapSessionId(s.session_id, suffix);
  ...
  const symptomRows = symptomRecordsJson.symptom_records.map((r) => ({
    symptom_id: remapSymptomId(r.symptom_id, suffix),
    patient_id: patientId,
  ...
  // ── 4. ai_results 삽입 ───────────────────────────────────────
  const latestSessionId = remapSessionId('ses_007', suffix);
