...
const DATA_DIR = path.join(process.cwd(), 'src/app/projects/eum/_references/data/patient');

export async function loadPatientData(patientId) {
    // 정적 JSON은 기존 fs 방식 유지
    const staticFiles = [
        '02_health_history.json',
        '04_medical_records.json',
        '07_vitals_wearable.json',
    ];
...
export async function stage2Normalize(entities) {
    const targets = entities.filter((e) => e.type === 'symptom' || e.type === 'diagnosis');
...
export async function stage3Candidates(mappings, entities) {
    ...
    const candidates = candidateArrays
        ...
        .filter(
            (c) => c.icd_code && c.disease_name && !seen.has(c.icd_code) && seen.add(c.icd_code)
        )
        .slice(0, 15);
