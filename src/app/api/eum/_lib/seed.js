...
const DATA_PATIENT = path.join(
  __dirname,
  '../../../projects/eum/_references/data/patient'
);
const DATA_DOCTOR = path.join(
  __dirname,
  '../../../projects/eum/_references/data/doctor'
);
...
async function seedSymptomRecords() {
  ...
  const { error } = await supabase.from('symptom_records').upsert(rows, { onConflict: 'symptom_id' });
  if (error) throw error;
